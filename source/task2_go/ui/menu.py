...
class MenuScene:
    def __init__(self, screen):
        ...
        self.choice=None; self.ai_depth=2; self.human_color=1

    def run_once(self, events):
        self.screen.fill((30,30,30))
        y=100
        items=[
            ("[1] Người vs Người", "pvp"),
            ("[2] Người vs Máy", "vsai"),
            (f"Độ sâu AI: {self.ai_depth}  (+/-)", None),
            (f"Người chơi: {'Đen' if self.human_color==1 else 'Trắng'}  (C để đổi)", None),
            ("Enter để bắt đầu", None),
        ]
        for text,_ in items:
